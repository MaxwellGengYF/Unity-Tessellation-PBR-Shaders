...
Shader "Tessellation/Standard" {
	Properties {
		...
		_ParallaxScale("Parallax Scale", Range(0,1)) = 0.01
...
struct appdata_tess{
	...
	inline void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			o.Metallic = _SpecularColor  * spec.r;
			o.Smoothness = _Glossiness * spec.a;
			#else
			o.Metallic = _SpecularColor;
...
inline float2 parallax_mapping(float3 tangentViewDir, float2 uv)
{
	// get height from height map (which is, parallax map)
	float height = tex2D(_HeightMap, uv).r -0.5;
	// get the P vector (check this out: http://learnopengl.com/#!Advanced-Lighting/Parallax-Mapping)
	float2 P = tangentViewDir.xy / tangentViewDir.z * (height * _ParallaxScale);
	return P;
}
...
inline void frag_surf (v2f_surf IN,
  ...
  SurfaceOutputStandard o;
    ...
    float3x3 wdMatrix= float3x3(  normalize(IN.tSpace0.xyz),  normalize(IN.tSpace1.xyz),  normalize(IN.tSpace2.xyz));
  // call surface function
 float2 offset = parallax_mapping((mul(IN.worldViewDir, wdMatrix)), IN.pack0.xy);
  surfIN.uv_MainTex -= offset;
  surf (surfIN, o);
  o.Normal = normalize(mul(wdMatrix, o.Normal));
